...
export const getTeritagePlanApi = (ownerAddress: string): Promise<ApiResponse> =>
  client.get(`/teritages/${ownerAddress}`).then((response) => response.data);
...
export const recordCheckInApi = (
  ownerAddress: string,
  payload: { triggeredBy?: string; note?: string; timestamp?: string } = {}
): Promise<ApiResponse> => client.post(`/teritages/${ownerAddress}/checkins`, payload).then((response) => response.data);

export const recordClaimApi = (
  ownerAddress: string,
  payload: { initiatedBy: string; note?: string }
): Promise<ApiResponse> => client.post(`/teritages/${ownerAddress}/claims`, payload).then((response) => response.data);
