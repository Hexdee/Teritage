...
const beneficiarySchema = z.object({
  ...
  sharePercentage: z.coerce
    .number()
    .min(1, { message: 'Share must be at least 1%' })
    .max(100, { message: 'Share cannot exceed 100%' }),
...
const formSchema = z
  ...
  .superRefine((values, ctx) => {
    const total = values.beneficiaries.reduce(
      (acc, item) => acc + item.sharePercentage,
      0
    );
...
export default function BeneficiaryInfoForm({ handleNext }: INextPage) {
  ...
  const totalShare = form
    .watch('beneficiaries')
    .reduce((acc, item) => acc + (Number(item.sharePercentage) || 0), 0);
                    ...
                    <Dialog>
                      ...
                      <DialogContent className='sm:max-w-sm'>
                        <DialogHeader>
                          ...
                          <Separator />
                          <DialogDescription>
                            EVM addresses start with 0x and identify wallets on
                            Ethereum-compatible networks such as Hedera smart
                            contracts, Base, or Polygon.
                          </DialogDescription>
                        </DialogHeader>
                      </DialogContent>
                    </Dialog>
                    ...
                    <Dialog>
                      ...
                      <DialogContent className='sm:max-w-sm'>
                        <DialogHeader>
                          ...
                          <Separator />
                          <DialogDescription>
                            If enabled, Teritage will send an email letting the
                            beneficiary know they have been added once your plan
                            is confirmed.
                          </DialogDescription>
                        </DialogHeader>
                      </DialogContent>
