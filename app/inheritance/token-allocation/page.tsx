...
import {
  BeneficiaryEntry,
  useInheritancePlanStore,
} from '@/store/useInheritancePlanStore';
import { useTeritageContract } from '@/lib/blockchain/useTeritageContract';
import {
  SUCCESS_ALLOCATION_URL,
  BENEFICIARY_INFO_URL,
  INHERITANCE_SETUP_URL,
} from '@/config/path';
...
const formSchema = z
  .object({
    tokens: z
      .array(tokenSchema)
      .min(1, { message: 'Add at least one token to manage' }),
  })
...
const formatName = (beneficiary: BeneficiaryEntry) =>
...
export default function TokenAllocation() {
  ...
  const summaryTotal = beneficiaries.reduce(
    (acc, beneficiary) => acc + beneficiary.sharePercentage,
    0
  );
  ...
  async function onSubmit(values: FormValues) {
    ...
    if (!checkInIntervalSeconds) {
      toast.error(
        'Check-in interval is missing. Please restart the setup process.'
      );
      router.replace(INHERITANCE_SETUP_URL);
      return;
    }
    ...
    try {
      const normalizedTokens = values.tokens.map((token) => ({
        ...
        address:
          token.type === 'HBAR'
            ? ZERO_ADDRESS
            : getAddress(token.address as string),
      }));

      const inheritorAddresses = beneficiaries.map((beneficiary) =>
        getAddress(beneficiary.walletAddress)
      );
      const shares = beneficiaries.map((beneficiary) =>
        Math.round(beneficiary.sharePercentage * 100)
      );
      ...
      const shouldNotifyBeneficiaries = beneficiaries.some(
        (beneficiary) => beneficiary.notifyBeneficiary
      );
      ...
      const backendPayload =
        normalizedOwnerProfile && normalizedOwnerProfile.name && normalizedOwnerProfile.email
          ? {
              ...
              checkInIntervalSeconds,
              ...(sanitizedSocialLinks.length
                ? { socialLinks: sanitizedSocialLinks }
                : {}),
              ...(shouldNotifyBeneficiaries
      ...
      const message =
        error instanceof Error
          ? error.message
          : 'Failed to save inheritance plan';
      toast.error(message);
    }
  }
      ...
      <div className='space-y-2'>
        ...
        <div className='space-y-4 rounded-lg border p-4'>
          {beneficiaries.map((beneficiary, index) => (
            <div
              key={`${beneficiary.walletAddress}-${index}`}
              className='grid grid-cols-2 gap-4 text-sm'
            >
              <div>
                <p className='text-muted-foreground'>Name</p>
                <p className='text-inverse font-medium'>
                  {formatName(beneficiary)}
                </p>
              </div>
              ...
              <div>
                ...
                <p className='text-inverse'>{beneficiary.email}</p>
              </div>
              <div className='text-right'>
                <p className='text-muted-foreground'>Wallet</p>
                <p className='font-mono text-xs text-inverse'>
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder={
                              isHbar ? 'HBAR uses the native balance' : '0x...'
                            }
                            disabled={isHbar}
                            {...field}
                          />
