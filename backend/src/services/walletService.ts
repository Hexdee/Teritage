...
export async function getWalletSummary(ownerIdentifier: string) {
  ...
  const plan = planDoc
    ? (planDoc.toObject({ versionKey: false }) as ITeritagePlan)
    : undefined;

  const totalBalanceUsd = tokens.reduce(
    (acc: number, token) => acc + token.priceUsd,
    0
  );
  const assignedPercentage = plan
    ? plan.inheritors.reduce(
        (acc: number, inheritor) => acc + inheritor.sharePercentage,
        0
      )
    : 0;
  ...
  return {
    ...
    change24hPercent: tokens.reduce(
      (acc: number, token) => acc + token.change24hPercent,
      0
    ),
    assignedPercentage,
